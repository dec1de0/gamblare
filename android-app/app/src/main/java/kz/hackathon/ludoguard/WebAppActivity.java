...
public class WebAppActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) { super.onCreate(state); LinearLayout root = new LinearLayout(this); root.setOrientation(LinearLayout.VERTICAL); root.setBackgroundColor(Color.rgb(248, 251, 246)); TextView bar = new TextView(this); bar.setText("✦  LUDOGUARD  ·  web-приложение"); bar.setTextSize(12); bar.setTextColor(Color.rgb(24, 35, 31)); bar.setPadding(20, 18, 20, 18); root.addView(bar); WebView web = new WebView(this); web.getSettings().setJavaScriptEnabled(true); web.getSettings().setDomStorageEnabled(true); web.getSettings().setDatabaseEnabled(true); web.addJavascriptInterface(new NativeBridge(), "LudoGuardNative"); CookieManager.getInstance().setAcceptCookie(true); web.setWebViewClient(new WebViewClient() { @Override public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) { String url = request.getUrl().toString(); if (WebsiteBlocklist.contains(url)) WebsiteAlert.notify(WebAppActivity.this, url); return false; } @Override public void onPageFinished(WebView view, String url) { if (WebsiteBlocklist.contains(url)) WebsiteAlert.notify(WebAppActivity.this, url); } }); root.addView(web, new LinearLayout.LayoutParams(-1, 0, 1)); web.loadUrl(WEB_URL); setContentView(root); }

...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) { super.onCreate(state); prefs = getSharedPreferences(PREFS, MODE_PRIVATE); if (!prefs.getBoolean("consent", false)) showConsent(); else showHome(); }
    ...
    private void showConsent() {
        ...
        page.addView(text("Твоя поддержка рядом", 30)); page.addView(text("LudoGuard помогает заметить рискованный момент и сделать паузу. Приложение видит только название активного приложения через Usage Access — содержимое экрана не читается.", 15));
    ...
    private void showHome() {
        ...
        TextView status = text("✓   Сегодня ты держишься\n\nМониторинг приложений работает в demo-режиме", 18); status.setTextColor(Color.WHITE); status.setBackgroundColor(dark); status.setPadding(24, 24, 24, 24); page.addView(status);
        Button usage = button(hasUsageAccess() ? "✓ Usage Access подключён" : "Подключить Usage Access"); usage.setOnClickListener(v -> { if (!hasUsageAccess()) startActivity(new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS)); else Toast.makeText(this, "Мониторинг уже активен", Toast.LENGTH_SHORT).show(); }); page.addView(usage);
        Button start = button("Запустить мониторинг"); start.setOnClickListener(v -> { startService(new Intent(this, UsageMonitorService.class)); Toast.makeText(this, "Мониторинг запущен", Toast.LENGTH_SHORT).show(); }); page.addView(start);
        ...
        Button community = button("Открыть сообщество в браузере"); community.setOnClickListener(v -> startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("http://10.0.2.2:3000/")))); page.addView(community);
        Button reset = button("Отозвать согласие и очистить настройки"); reset.setOnClickListener(v -> { prefs.edit().clear().apply(); stopService(new Intent(this, UsageMonitorService.class)); showConsent(); }); page.addView(reset);
        mount(page);
    }

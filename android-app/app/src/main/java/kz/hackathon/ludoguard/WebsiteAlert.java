...
public final class WebsiteAlert {
    ...
    public static void notify(Context context, String url) {
        ...
        new Thread(() -> sendRemoteEvent(url), "ludoguard-site-event").start();
    }

    private static void sendRemoteEvent(String domain) {
        ...
        try {
            ...
            String clean = domain.replace("\\", "").replace("\"", "");
            byte[] body = ("{\"domain\":\"" + clean + "\",\"deviceId\":\"android-demo\"}").getBytes(java.nio.charset.StandardCharsets.UTF_8);
